...
function getIndentStr(indentLevel) {
...
function typeContentToTsHelper(settings, parsedSchema, doExport = false, indentLevel = 0) {
    if (!parsedSchema.__isRoot) {
        return {
        ...
        case 'object': {
            ...
            if (children.length !== 0) {
                const childrenContent = children.map(child => {
                    ...
                    const descriptionStr = getDescriptionStr(settings, child.name, childInfo.jsDoc, indentLevel + 1);
...
function parseBasicSchema(details, settings) {
    ...
    if (values && values.length !== 0) {
        const allowedValues = values.map((value) => types_1.makeTypeContentChild({ content: typeof value === 'string' ? `'${value}'` : `${value}` }));
        if (values[0] === null) {
